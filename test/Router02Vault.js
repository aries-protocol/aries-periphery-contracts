const { expectEqual, expectEvent, expectRevert, expectAlmostEqualMantissa, bnMantissa, BN } = require("./Utils/JS");
const { address, increaseTime, encode } = require("./Utils/Ethereum");
const { getAmounts, leverage, permitGenerator } = require("./Utils/AriesPeriphery");
...
const UniswapV2Factory = artifacts.require("test/Contracts/sushi/UniswapV2Factory.sol:UniswapV2Factory");
const UniswapV2Router02 = artifacts.require("test/Contracts/sushi/UniswapV2Router02.sol:UniswapV2Router02");
const UniswapV2Pair = artifacts.require("test/Contracts/uniswap-v2-core/UniswapV2Pair.sol:UniswapV2Pair");
const AriesPriceOracle = artifacts.require("AriesPriceOracle");
...
const MasterChef = artifacts.require("test/Contracts/sushi/MasterChef.sol:MasterChef");
...
contract("Router02 Vault", function (accounts) {
    ...
    before(async () => {
        ...
        ariesPriceOracle = await AriesPriceOracle.new();
        const bDeployer = await BDeployer.new();
        const cDeployer = await CDeployer.new();
        ariesFactory = await Factory.new(address(0), address(0), bDeployer.address, cDeployer.address, ariesPriceOracle.address);
        WETH = await WETH9.new();
        uniswapV2Router02 = await UniswapV2Router02.new(uniswapV2Factory.address, WETH.address);
        router = await Router02.new(ariesFactory.address, bDeployer.address, cDeployer.address, WETH.address);

        masterChef = await MasterChef.new(rewardsToken.address, address(0), 0, 0);

        vaultTokenFactory = await VaultTokenFactory.new(uniswapV2Router02.address, masterChef.address, rewardsToken.address, 998);
        // Create Uniswap Pair
        ...
        // Create Pair On Aries
        collateralAddress = await ariesFactory.createCollateral.call(vaultTokenAddress);
        borrowable0Address = await ariesFactory.createBorrowable0.call(vaultTokenAddress);
        borrowable1Address = await ariesFactory.createBorrowable1.call(vaultTokenAddress);
        await ariesFactory.createCollateral(vaultTokenAddress);
        await ariesFactory.createBorrowable0(vaultTokenAddress);
        await ariesFactory.createBorrowable1(vaultTokenAddress);
        await ariesFactory.initializeLendingPool(vaultTokenAddress);
    ...
    it("optimal liquidity", async () => {
        ...
        await expectRevert(
            ...
            ETH_IS_A ? "AriesRouter: INSUFFICIENT_B_AMOUNT" : "AriesRouter: INSUFFICIENT_A_AMOUNT"
        );
        ...
        await expectRevert(
            ...
            ETH_IS_A ? "AriesRouter: INSUFFICIENT_A_AMOUNT" : "AriesRouter: INSUFFICIENT_B_AMOUNT"
        );
    });
    ...
    it("mint", async () => {
        ...
        expect((await collateral.balanceOf(borrower)) * 1).to.eq(LP_AMOUNT.sub(MINIMUM_LIQUIDITY.mul(new BN(2))) * 1);
    });
    ...
    it("redeem", async () => {
        ...
        //Redeem UNI
        await expectRevert(
            router.redeem(borrowableUNI.address, UNI_REDEEM_AMOUNT, lender, "0", "0x", { from: lender }),
            "AriesRouter: EXPIRED"
        );
        await expectRevert(
            router.redeem(borrowableUNI.address, UNI_REDEEM_AMOUNT, lender, DEADLINE, "0x", { from: lender }),
            "Aries: TRANSFER_NOT_ALLOWED"
        );
        ...
        //Redeem ETH
        await expectRevert(
            router.redeemETH(borrowableUNI.address, UNI_REDEEM_AMOUNT, lender, DEADLINE, "0x", { from: lender }),
            "AriesRouter: NOT_WETH"
        );
        await expectRevert(
            router.redeemETH(borrowableWETH.address, ETH_REDEEM_AMOUNT, lender, "0", "0x", { from: lender }),
            "AriesRouter: EXPIRED"
        );
        await expectRevert(
            router.redeemETH(borrowableWETH.address, ETH_REDEEM_AMOUNT, lender, DEADLINE, "0x", { from: lender }),
            "Aries: TRANSFER_NOT_ALLOWED"
        );
        ...
        //Redeem LP
        const permitRedeemLP = await permitGenerator.permit(collateral, borrower, router.address, LP_REDEEM_AMOUNT, DEADLINE);
        await router.redeem(collateral.address, LP_REDEEM_AMOUNT, borrower, DEADLINE, permitRedeemLP, { from: borrower });
        expect((await uniswapV2Pair.balanceOf(borrower)) * 1).to.eq(LP_REDEEM_AMOUNT * 1);
        ...
        const permitData = await permitGenerator.permit(uniswapV2Pair, borrower, router.address, LP_REDEEM_AMOUNT, DEADLINE);
        await router.mintCollateral(collateral.address, LP_REDEEM_AMOUNT, borrower, DEADLINE, permitData, { from: borrower });
    });

    it("borrow", async () => {
        //Borrow UNI
        await expectRevert(
            router.borrow(borrowableUNI.address, UNI_BORROW_AMOUNT, borrower, "0", "0x", { from: borrower }),
            "AriesRouter: EXPIRED"
        );
        await expectRevert(
            router.borrow(borrowableUNI.address, UNI_BORROW_AMOUNT, borrower, DEADLINE, "0x", { from: borrower }),
            "Aries: BORROW_NOT_ALLOWED"
        );
        ...
        //Borrow ETH
        await expectRevert(
            router.borrowETH(borrowableUNI.address, UNI_BORROW_AMOUNT, borrower, DEADLINE, "0x", { from: borrower }),
            "AriesRouter: NOT_WETH"
        );
        await expectRevert(
            router.borrowETH(borrowableWETH.address, ETH_BORROW_AMOUNT, borrower, "0", "0x", { from: borrower }),
            "AriesRouter: EXPIRED"
        );
        await expectRevert(
            router.borrowETH(borrowableWETH.address, ETH_BORROW_AMOUNT, borrower, DEADLINE, "0x", { from: borrower }),
            "Aries: BORROW_NOT_ALLOWED"
        );
    ...
    it("repay", async () => {
        ...
        const actualRepayETH = await router.repayETH.call(borrowableWETH.address, borrower, DEADLINE, {
            value: ETH_REPAY_AMOUNT1,
            from: borrower,
        });
    ...
    it("leverage", async () => {
        await expectRevert(
            leverage(router, vaultToken, borrower, "100", "8000", "90", "7000", "0x", "0x", ETH_IS_A),
            ETH_IS_A ? "AriesRouter: INSUFFICIENT_A_AMOUNT" : "AriesRouter: INSUFFICIENT_B_AMOUNT"
        );
        await expectRevert(
            leverage(router, vaultToken, borrower, "80", "10000", "70", "9000", "0x", "0x", ETH_IS_A),
            ETH_IS_A ? "AriesRouter: INSUFFICIENT_B_AMOUNT" : "AriesRouter: INSUFFICIENT_A_AMOUNT"
        );
        await expectRevert(
            leverage(router, vaultToken, borrower, ETH_LEVERAGE_AMOUNT, UNI_LEVERAGE_AMOUNT, "0", "0", "0x", "0x", ETH_IS_A),
            "Aries: BORROW_NOT_ALLOWED"
        );
    ...
    it("liquidate", async () => {
        ...
        await expectRevert.unspecified(
            router.liquidate(borrowableUNI.address, UNI_LIQUIDATE_AMOUNT, borrower, liquidator, DEADLINE, { from: liquidator })
        );
        ...
        await expectRevert(
            router.liquidateETH(borrowableUNI.address, borrower, liquidator, DEADLINE, { value: ETH_LIQUIDATE_AMOUNT, from: liquidator }),
            "AriesRouter: NOT_WETH"
        );
        await expectRevert(
        ...
        const liquidateETHResult = await router.liquidateETH.call(borrowableWETH.address, borrower, liquidator, DEADLINE, {
            value: ETH_LIQUIDATE_AMOUNT,
            from: liquidator,
        });
        const op = router.liquidateETH(borrowableWETH.address, borrower, liquidator, DEADLINE, {
            value: ETH_LIQUIDATE_AMOUNT,
            from: liquidator,
        });
        ...
        const op2 = router.liquidateETH(borrowableWETH.address, borrower, liquidator, DEADLINE, {
            value: ETH_LIQUIDATE_AMOUNT2,
            from: liquidator,
        });
        await checkETHBalance(op2, liquidator, expectedETHAmount, true);
    });

    it("ariesBorrow is forbidden to non-borrowable", async () => {
        // Fails because data cannot be empty
        await expectRevert.unspecified(router.ariesBorrow(router.address, address(0), "0", "0x"));
        const data = encode(["uint8", "address", "uint8", "bytes"], [0, vaultToken.address, 0, "0x"]);
        // Fails becasue msg.sender is not a borrowable
        await expectRevert(router.ariesBorrow(router.address, address(0), "0", data), "AriesRouter: UNAUTHORIZED_CALLER");
        // Fails because sender is not the router
        const borrowableA = ETH_IS_A ? borrowableWETH : borrowableUNI;
        await expectRevert(borrowableA.borrow(borrower, router.address, "0", data), "AriesRouter: SENDER_NOT_ROUTER");
    });

    it("ariesRedeem is forbidden to non-collateral", async () => {
        // Fails because data cannot be empty
        await expectRevert.unspecified(router.ariesRedeem(router.address, "0", "0x"));
        const data = encode(["uint8", "address", "uint8", "bytes"], [0, vaultToken.address, 0, "0x"]);
        // Fails becasue msg.sender is not a borrowable
        await expectRevert(router.ariesRedeem(router.address, "0", data), "AriesRouter: UNAUTHORIZED_CALLER");
        // Fails because sender is not the router
        await expectRevert(collateral.flashRedeem(router.address, "0", data), "AriesRouter: SENDER_NOT_ROUTER");
    });
    ...
    it("address calculation", async () => {
        ...
        expect(await router.getBorrowable(vaultToken.address, "0")).to.eq(expectedBorrowableA);
        expect(await router.getBorrowable(vaultToken.address, "1")).to.eq(expectedBorrowableB);
        expect(await router.getCollateral(vaultToken.address)).to.eq(expectedCollateral);
        ...
        const receipt = await router.getBorrowable.sendTransaction(vaultToken.address, "0");
    ...
    it("max approve", async () => {
        //Redeem ETH
        await expectRevert(
            router.redeemETH(borrowableWETH.address, MAX_APPROVE_ETH_AMOUNT, lender, DEADLINE, "0x", { from: lender }),
            "Aries: TRANSFER_NOT_ALLOWED"
        );
