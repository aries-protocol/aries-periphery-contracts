...
const { getAmounts, leverage, deleverage, permitGenerator } = require("./Utils/AriesPeriphery");
...
contract("Deleverage02 Vault", function (accounts) {
    ...
    beforeEach(async () => {
        ...
        masterChef = await MasterChef.new(rewardsToken.address, address(0), 0, 0, 0);
        ...
        // Create pool
        await masterChef.add(100, uniswapV2PairAddress, true);
        // Create vaultToken
    ...
    it("deleverage", async () => {
        ...
        await expectRevert(
            deleverage(router, vaultToken, borrower, LP_DLVRG_TOKENS, ETH_DLVRG_MIN, UNI_DLVRG_MIN, "0x", ETH_IS_A),
        ...
        await expectRevert(
            deleverage(router, vaultToken, borrower, "0", ETH_DLVRG_MIN, UNI_DLVRG_MIN, permit, ETH_IS_A),
            "AriesRouter: REDEEM_ZERO"
        );
        await expectRevert(
            deleverage(router, vaultToken, borrower, LP_DLVRG_TOKENS, ETH_DLVRG_HIGH, UNI_DLVRG_MIN, permit, ETH_IS_A),
            ETH_IS_A ? "AriesRouter: INSUFFICIENT_A_AMOUNT" : "AriesRouter: INSUFFICIENT_B_AMOUNT"
        );
        await expectRevert(
            deleverage(router, vaultToken, borrower, LP_DLVRG_TOKENS, ETH_DLVRG_MIN, UNI_DLVRG_HIGH, permit, ETH_IS_A),
        ...
        const receipt = await deleverage(router, vaultToken, borrower, LP_DLVRG_TOKENS, ETH_DLVRG_MIN, UNI_DLVRG_MIN, permit, ETH_IS_A);
    ...
    it("deleverage with refund UNI", async () => {
        ...
        const receipt = await deleverage(router, vaultToken, borrower, LP_DLVRG_TOKENS, "0", "0", permit, ETH_IS_A);
