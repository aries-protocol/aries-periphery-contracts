...
import "./AriesERC20.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IPoolToken.sol";
import "./libraries/SafeMath.sol";

contract PoolToken is IPoolToken, AriesERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Aries: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
    ...
    function mint(address minter) external nonReentrant update returns (uint256 mintTokens) {
        ...
        require(mintTokens > 0, "Aries: MINT_AMOUNT_ZERO");
    ...
    function redeem(address redeemer) external nonReentrant update returns (uint256 redeemAmount) {
        ...
        require(redeemAmount > 0, "Aries: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Aries: INSUFFICIENT_CASH");
    ...
    function _safeTransfer(address to, uint256 amount) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Aries: TRANSFER_FAILED");
    }

    // prevents a contract from calling itself, directly or indirectly.
    bool internal _notEntered = true;
    modifier nonReentrant() {
        require(_notEntered, "Aries: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
