...
import "./interfaces/IAriesPriceOracle.sol";

contract AriesPriceOracle is IAriesPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "AriesPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(!pairStorage.initialized, "AriesPriceOracle: ALREADY_INITIALIZED");
    ...
    function getResult(address uniswapV2Pair) external returns (uint224 price, uint32 T) {
        ...
        require(pair.initialized, "AriesPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "AriesPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
