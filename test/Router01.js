...
contract("Router01", function (accounts) {
    ...
    before(async () => {
        ...
        await ariesPriceOracle.initialize(uniswapV2PairAddress);
        collateralAddress = await ariesFactory.createCollateral.call(uniswapV2PairAddress);
        borrowable0Address = await ariesFactory.createBorrowable0.call(uniswapV2PairAddress);
        borrowable1Address = await ariesFactory.createBorrowable1.call(uniswapV2PairAddress);
        await ariesFactory.createCollateral(uniswapV2PairAddress);
        await ariesFactory.createBorrowable0(uniswapV2PairAddress);
        await ariesFactory.createBorrowable1(uniswapV2PairAddress);
        await ariesFactory.initializeLendingPool(uniswapV2PairAddress);
        ...
        router = await Router01.new(ariesFactory.address, bDeployer.address, cDeployer.address, WETH.address);
    ...
    it("liquidate", async () => {
        ...
        await ariesPriceOracle.getResult(uniswapV2Pair.address);
        await expectRevert(
    ...
    it("ariesBorrow is forbidden to non-borrowable", async () => {
        ...
        const data = encode(["uint8", "address", "uint8", "bytes"], [0, uniswapV2Pair.address, 0, "0x"]);
