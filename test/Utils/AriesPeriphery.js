...
async function getPermit(opts) {
    ...
    const digest = await getApprovalDigest(name, token.address, { owner, spender, value }, nonce, deadline, borrowPermit);
...
const permitGenerator = {
    ...
    _permit: async (token, owner, spender, value, deadline, borrowPermit) => {
        if (permitGenerator.activatePermit) {
            ...
            return encode(["bool", "uint8", "bytes32", "bytes32"], [value.eq(MAX_UINT_256), v, r, s]);
        } else {
