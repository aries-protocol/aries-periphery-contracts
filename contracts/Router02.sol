...
import "./interfaces/IAriesCallee.sol";
...
contract Router02 is IRouter02, IAriesCallee {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "AriesRouter: EXPIRED");
        _;
    }

    modifier checkETH(address poolToken) {
        require(WETH == IPoolToken(poolToken).underlying(), "AriesRouter: NOT_WETH");
        _;
    }
    ...
    function mintETH(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override ensure(deadline) checkETH(poolToken) returns (uint256 tokens) {
    ...
    function redeemETH(
        ...
        uint256 deadline,
        bytes memory permitData
    ) public virtual override ensure(deadline) checkETH(poolToken) returns (uint256 amountETH) {
    ...
    function repayETH(
        ...
        address borrower,
        uint256 deadline
    ) external payable virtual override ensure(deadline) checkETH(borrowable) returns (uint256 amountETH) {
    ...
    function liquidate(
        ...
        address to,
        uint256 deadline
    ) external virtual override ensure(deadline) returns (uint256 amount, uint256 seizeTokens) {
        amount = _repayAmount(borrowable, amountMax, borrower);
        TransferHelper.safeTransferFrom(IBorrowable(borrowable).underlying(), msg.sender, borrowable, amount);
    ...
    function liquidateETH(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override ensure(deadline) checkETH(borrowable) returns (uint256 amountETH, uint256 seizeTokens) {
        amountETH = _repayAmount(borrowable, msg.value, borrower);
        IWETH(WETH).deposit{value: amountETH}();
        assert(IWETH(WETH).transfer(borrowable, amountETH));
        seizeTokens = IBorrowable(borrowable).liquidate(borrower, to);
    ...
    function _leverage(
        ...
        bytes memory borrowAData = abi.encode(
            CalleeData({
                ...
                underlying: underlying,
                borrowableIndex: 0,
                data: abi.encode(BorrowBCalldata({borrower: msg.sender, receiver: address(this), borrowAmount: amountB, data: borrowBData}))
            })
        );
        // borrow borrowableA
    ...
    function leverage(
        ...
        (uint256 amountA, uint256 amountB) = _optimalLiquidity(uniswapV2Pair, amountADesired, amountBDesired, amountAMin, amountBMin);
    ...
    function _addLiquidityAndMint(
        ...
        // mint LP token
        if (isVaultToken(underlying)) IUniswapV2Pair(uniswapV2Pair).mint(underlying);
    ...
    function deleverage(
        ...
        require(redeemTokens > 0, "AriesRouter: REDEEM_ZERO");
    ...
    function _removeLiqAndRepay(
        ...
        if (isVaultToken(underlying)) IVaultToken(underlying).redeem(uniswapV2Pair);
        (uint256 amountAMax, uint256 amountBMax) = IUniswapV2Pair(uniswapV2Pair).burn(address(this));
        require(amountAMax >= amountAMin, "AriesRouter: INSUFFICIENT_A_AMOUNT");
        require(amountBMax >= amountBMin, "AriesRouter: INSUFFICIENT_B_AMOUNT");
        // repay and refund
    ...
    function ariesBorrow(
        ...
        require(sender == address(this), "AriesRouter: SENDER_NOT_ROUTER");
        require(msg.sender == declaredCaller, "AriesRouter: UNAUTHORIZED_CALLER");
    ...
    function _optimalLiquidity(
        ...
        if (amountBOptimal <= amountBDesired) {
            require(amountBOptimal >= amountBMin, "AriesRouter: INSUFFICIENT_B_AMOUNT");
            (amountA, amountB) = (amountADesired, amountBOptimal);
        } else {
            uint256 amountAOptimal = _quote(amountBDesired, reserveB, reserveA);
            assert(amountAOptimal <= amountADesired);
            require(amountAOptimal >= amountAMin, "AriesRouter: INSUFFICIENT_A_AMOUNT");
    ...
    function _quote(
        ...
        require(amountA > 0, "AriesRouter: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "AriesRouter: INSUFFICIENT_LIQUIDITY");
        amountB = amountA.mul(reserveB) / reserveA;
    }

    function isVaultToken(address underlying) public view virtual override returns (bool) {
    ...
    function getBorrowable(address underlying, uint8 index) public view virtual override returns (address borrowable) {
        require(index < 2, "AriesRouter: INDEX_TOO_HIGH");
        borrowable = address(
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"8288d9477e2eadaf3b172365a1481b73dd1fbf4bfe70fa07056089111a554810" // Borrowable bytecode keccak256
                    )
                )
            )
        );
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"bb1e26ba4d7b99ce32187903f89f983c95fc479fc8acd1381f8697c7d6e81dc8" // Collateral bytecode keccak256
                    )
                )
            )
        );
